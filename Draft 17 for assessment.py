...
def choose_character():
    ...
    if len(choice_list) == 1:
        eg.buttonbox("Choose your character", title="Choose your character",
                     choices=choice_list)
...
def get_input(name, category_name, category_list):
    ...
    for attr in category_list:
        value = eg.integerbox(f"What is your {attr}?", title="Input values")
        if value is None:
            return
...
eg.msgbox("Welcome to Ollie's awesome character creator!", title="Welcome")
main_menu()
